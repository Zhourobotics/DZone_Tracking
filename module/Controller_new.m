classdef Controller_new < handle
    properties
    ...
    methods
        ...
        function con = Controller_new(cfg)
            %% constructor 
            
            global pr model index
%              ROS_IP = 'localhost';
%             setenv('ROS_IP', ROS_IP);
%             con.node = ros2node("Controller");
...
%                    v2_ = reshape(v2_trunc, [3, model.nRobot]);
                   ...
                   v = 10 * (v1_ ./ v1_norm) + 10 * (v2_ ./ v2_norm); % changed v2 to 10
                   v = 0.01 * v;
...
%                   disp("before min");
%                   disp(max_neighbor_dist);
                  max_neighbor_dist = min(max_neighbor_dist, model.max_dist);
%                   disp("after min");
%                   disp(max_neighbor_dist);
%                   disp("min")
                  cons = ai_normalized * ai'  - model.delta(iZone) * max_neighbor_dist- erf_value * sqrt(sqr_term);
%                   erf_value
        ...
        function flags = if_in_zone_sensor(con)
            ...
            for iRobot = 1:con.nRobot_
                for iZone = 1:con.nTypeI_
                    ...
                    if drone_dist < 0.85*model.typeI_d(iZone) ...
                            && flags(iRobot, iZone) == 0
                        flags(iRobot, iZone) = 1;
                    elseif drone_dist > model.typeI_d(iZone) ...
                            && flags(iRobot, iZone) == 1
